...
my %static_types = (
  ...
  # start receives?
  S_RGB_LIGHT             => { receives => [], sends => [V_RGB_V_WATT] }, # RGB light
  S_RGBW_LIGHT            => { receives => [], sends => [V_RGBW,V_WATT] }, # RGBW light (with separate white component)
  # end receives?
  S_COLOR_SENSOR          => { receives => [], sends => [V_RGB] }, # Color sensor
